...
void main()
{
    ...
    bool spawn = false;
    if (p.w <= 0.0) {
        vec4 tmp = texture2D(spawnTexture, fract(v_Texcoord + elapsedTime));
        v.xy = tmp.zw;
        p.xy = tmp.xy;
        spawn = true;
    }
...
void main()
{
    ...
    vec4 p = texture2D(posTexture, v_Texcoord);
    if (p.w <= 0.0) {
        p.xy = texture2D(spawnTexture, fract(v_Texcoord + elapsedTime)).xy;
        p.w = life;
    }
    else {
        p.w -= deltaTime;
    ...
    p.z = v.z;
    p.xy += v.xy * deltaTime / 20.0 * speedScaling;
    gl_FragColor = p;
}
@end
